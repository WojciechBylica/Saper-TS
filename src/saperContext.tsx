...
const useSaper = () => {
  ...
  const isWon =
    ...
    return { count, setCount, bombsLeft, setBombsLeft, hydratedFields, setHydratedFields, gameLength, countDown, setCountDown,isExploded,startTimer, isDraw, isWon }
}

export const SaperContextProvider = ({ children }:{ children: ReactNode }) => {

    return (
        <SaperContext.Provider value={ useSaper()}>
            {children}
        </SaperContext.Provider>
    )
}