...
const useSaper = () => {
  ...
  const isWon =
    countDown > 0 &&
    hydratedFields.filter((field) => !field.bomb).every((field) => field.state === 'clicked') &&
    hydratedFields.filter((field) => field.bomb).every(({ state }) => state !== 'exploded')
...
export const SaperContextProvider = ({ children }: { children: ReactNode }) => {
  return <SaperContext.Provider value={useSaper()}>{children}</SaperContext.Provider>
}
